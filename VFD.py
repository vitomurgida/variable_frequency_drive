...
n_poles = 48 # motor number of poles
...
N_candidates = 10
f_sw_choices = np.array([2400])  # [Hz] - if multiple values defined, an optimization will be run and the best switching frequency will be calcualted
m_min, m_max = 0.05, 0.95 # for optimization, no need to change

# ---------------- PARAMETERS ----------------
V_ll_rms = 400.0 # line to line input voltage rms
f_grid = 60.0
w_grid = 2 * np.pi * f_grid
V_phase_peak = np.sqrt(2) * V_ll_rms / np.sqrt(3)

R_s = 0.05 # system resistance
L_s = 5e-6 # filter inductance
C_dc = 5e-2 # DC bus capacitance
V_diode = 4 # diode threshold voltage
...
# ---------------- TIME ----------------
freq_microp = 40e3 # Hz microprocessor sampling frequency
t_end = 30*1/f_grid
dt = 1/freq_microp
